...
final class IGStoryPreviewCell: UICollectionViewCell, UIScrollViewDelegate {
                ...
                case .forward:
                    if snapIndex < story?.snapsCount ?? 0 {
                        if let snap = story?.snaps?[snapIndex] {
                            ...
                            storyHeaderView.lastUpdatedLabel.text = snap.lastUpdated.timeAgo()
    ...
    public var story: IGStory? {
        didSet {
            ...
            storyHeaderView.snaperImageView.downloadImageFromRefWithoutCache(ref: BaseClass().getProfilePicRef(of: story?.userId ?? ""), placeholder: "user")
        }
    }
