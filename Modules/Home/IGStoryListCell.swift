...
final class IGStoryListCell: UICollectionViewCell {
    ...
    public var story: IGStory? {
        didSet {
            self.profileNameLabel.text = story?.name
            self.profileImageView.imageView.downloadImageFromRefWithoutCache(ref: BaseClass().getProfilePicRef(of: story?.userId ?? ""), placeholder: "user")
        }
    }
    ...
    private let profileImageView: IGRoundedView = {
        ...
        roundedView.backgroundColor = UIColor(named: "backgroundColor")
